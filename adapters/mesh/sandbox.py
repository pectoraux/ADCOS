"""ADCOS mesh sandbox (WORK-023): the failure-isolation boundary.

:class:`SandboxedMesh` mediates EVERY call from the manager to a
mesh/relay implementation.  The mediator guarantees, mechanically
(mirroring the WORK-016 adapter, WORK-017 transport, WORK-018 IP
integration, WORK-019 5G Core integration, WORK-021 Wi-Fi, and
WORK-022 backhaul sandboxes):

1. **Exception isolation** -- any exception the implementation raises
   (``Exception`` AND ``BaseException``: a ``SystemExit`` from a
   vendor relay-firmware SDK crashes the operation, never the
   manager) is converted into a typed
   :class:`adapters.mesh.errors.MeshFailure` VALUE.  Mesh-side faults
   never propagate into core callers as exceptions (R5
   failure-isolation invariant).  Exception MESSAGE TEXT is
   deliberately NOT captured (LOCK-023: an implementation cannot
   leak relay management credentials, sidelink protection keys, or
   IAB donor authentication material through failure diagnostics);
   only the exception CLASS NAME crosses, as a vocabulary-free fact.
...
2. **Contract enforcement** -- every return value is validated against
   ...
   whose ref embeds WORK-012 session material (the W023 identity
   invariant: session_id != mesh link identity != route identity !=
   bearer identity != bundle identity) is rejected at the seam with
   the value discarded.  A route view whose path fingerprint does not
   bind to its ordinary-Path content is rejected at the seam.  A
   bundle view with a malformed ref, a verdict outside the frozen
   vocabulary, or an observation outside the generic WORK-016 metric
   vocabulary is rejected at the seam.  A leaky application-session
   facade that exposes ADCOS/mesh tokens (``session_id`` /
   ``bearer_ref`` / ``route_ref`` / ``bundle_ref`` / ...) as public
   attributes is rejected at the seam (LOCK-019 analog).

3. **Deterministic budget** -- each operation receives a step budget
   through the least-authority :class:`MeshContext`; spending beyond
   the budget is the deterministic model of a hung operation
   (``BUDGET_EXHAUSTED``).  There is no wall-clock timeout anywhere
   in the mesh layer.  The per-operation charge table is the frozen,
   ...
   fivegc/wifi/backhaul engine-side charging behavior.
...
The sandbox knows nothing about sessions, identity, relay
credentials, radio-link state machines, or PHY: it is pure mediation
between the manager and the implementation.

NOTE (the W023 authority path, architect-anchored): the sandbox
...
that cross this seam are the 16 mediated operations above (charged,
contract-validated, exception-isolated) and the LEAST-AUTHORITY
MeshContext facade.  An implementation that owns a REAL wire data
path encapsulates it INSIDE the MeshAppSession facade its mediated
``app_session`` operation returns (the facade owns its private data
path; the manager returns that facade verbatim with the egress
routing bound) -- exactly the accepted WORK-019/021/022 pattern.
"""
...
from .errors import MeshError, MeshFailure, MeshReasonCode
from .model import (
    BundleState,
    BundleView,
    ForwardOutcome,
...
# here so the sandbox can catch it.  (Mirrors the WORK-018/019/021/022
...
#: Default deterministic step budget (mirrors WORK-016/018/019/021/022).
...
#: The frozen deterministic step-charge table for the 16
#: :class:`~adapters.mesh.contract.MeshContract` operations
#: (op -> cost).  This is the family's PINNABLE surface: the selftest
#: pins this table byte-for-byte, and implementations charge these
#: costs against the :class:`~adapters.mesh.contract.MeshContext`
#: budget at op entry (mirroring the fivegc/wifi/backhaul
...
#: ADCOS/mesh tokens a leaky application-session facade must NOT
...
_LEAKY_APPSESSION_TOKENS = frozenset(
    {
        ...
        "route_ref", "path_ref", "bundle_ref", "allocation_ref",
        "external_link_id", "adcos", "mesh", "iab", "sidelink",
        "relay", "hop_budget",
    }
)
...
@dataclass
class MeshOpResult:
    """The mediated result of a mesh/relay operation.

    * ``ok=True``: ``value`` carries the validated contract return.
    * ``ok=False``: ``failure`` carries the typed, isolated
      :class:`MeshFailure` (never an exception).  ``detail`` is a
      generic, secret-free diagnostic string (exception message text
      is NEVER captured -- LOCK-023).

    Caller-side state errors (unknown binding, double close, duplicate
    bundle replay, queue exhaustion) RAISE :class:`MeshError` from the
    manager; adapter-side faults RETURN this typed value.
    """

    ok: bool
    value: Any = None
    failure: Optional[MeshFailure] = None
    detail: str = ""
...
class SandboxedMesh:
    """The failure-isolation mediator for a mesh/relay implementation.

    Constructed with a :class:`MeshContract` implementation (NOT
    ``hasattr`` duck-typed -- ``isinstance`` enforced) and the
    least-authority readers the manager injects.  Every public method
    builds a fresh :class:`MeshContext`, delegates to the
    ...
    def _context(self, now: str) -> MeshContext:
        return MeshContext(
        ...
        *,
        ...
        try:
            ...
            return MeshOpResult(
                ok=False,
                failure=MeshFailure(
                    reason_code=MeshReasonCode.BUDGET_EXHAUSTED,
                    integration_id=self._integration_id,
                    operation=operation,
                ),
                detail="mesh operation exceeded its deterministic "
            ...
            # captured -- an implementation cannot leak relay
            # management or sidelink key material through failure
            # diagnostics (LOCK-023).
            self._record_failure()
            return MeshOpResult(
                ok=False,
                failure=MeshFailure(
                ...
                detail="implementation raised MeshError (reason=%s); "
        ...
        if isinstance(validated, _ContractViolation):
            ...
            return MeshOpResult(
                ok=False,
                failure=MeshFailure(
                    reason_code=MeshReasonCode.CONTRACT_VIOLATION,
                    integration_id=self._integration_id,
                    operation=operation,
                ),
        ...
        return MeshOpResult(ok=True, value=validated)
    ...
    def _validate_link_view(self, value: Any) -> Any:
        if not isinstance(value, RelayLinkView):
            return _ContractViolation(
                "provision_link must return a RelayLinkView"
            )
        return value

    def _validate_route_view(self, value: Any) -> Any:
        if not isinstance(value, MeshRouteView):
            return _ContractViolation(
                "register_route must return a MeshRouteView"
            )
        # Ordinary-Path binding re-assert (the model enforces at
        # construction; the seam re-checks structurally so a hostile
        # subclass cannot smuggle a misbound route into manager
        # state).  The check recomputes the WORK-011 content-derived
        # fingerprint over the view's own content -- a mechanical
        # binding check, never a routing decision.
        try:
            value.path_ref  # noqa: B018 (shape touch)
        except AttributeError:
            return _ContractViolation(
                "register_route returned a malformed route view"
            )
        return value

    def _validate_allocation(self, value: Any) -> Any:
        if not isinstance(value, MeshAllocation):
            return _ContractViolation(
                "allocate must return a MeshAllocation"
            )
        return value

    def _validate_binding(self, value: Any) -> Any:
        if not isinstance(value, MeshBinding):
            return _ContractViolation(
                "bind_session must return a MeshBinding"
            )
        # W023 identity invariant, re-asserted at the seam: the
        ...
        try:
            ...
            return _ContractViolation(
                ...
                "malformed or embeds session identity (W023 identity "
                "invariant); value discarded"
            )
        # Structural content-derivation re-assert (mirrors the
        # WORK-022 PR #23 discipline): the binding key MUST equal
    ...
    def _validate_bundle_view(self, value: Any) -> Any:
        if not isinstance(value, BundleView):
            return _ContractViolation(
                "enqueue_bundle/inspect_bundle must return a BundleView"
            )
        # Bundle identity separation re-assert (the model enforces at
        # construction; the seam re-checks structurally).
        try:
            validate_opaque_ref(value.bundle_ref, "bundle")
            assert_ref_session_separation(value.bundle_ref, value.session_id)
        except MeshError:
            return _ContractViolation(
                "a bundle view's bundle_ref is malformed or embeds "
                "session identity (W023 identity invariant); value "
                "discarded"
            )
        return value

    def _validate_outcome(self, value: Any) -> Any:
        if not isinstance(value, ForwardOutcome):
            return _ContractViolation(
                "forward_bundle must return a ForwardOutcome"
            )
        # Verdict vocabulary + bundle identity separation re-assert
        # (the model enforces at construction; the seam re-checks).
        if value.verdict not in ForwardVerdict.values():
            return _ContractViolation(
                "forward_bundle returned a verdict %r outside the frozen "
                "vocabulary %s; value discarded"
                % (value.verdict, list(ForwardVerdict.values()))
            )
        try:
            validate_opaque_ref(value.bundle_ref, "bundle")
        except MeshError:
            return _ContractViolation(
                "forward_bundle returned a malformed bundle_ref; value "
                "discarded"
            )
        if value.state not in BundleState.values():
            return _ContractViolation(
                "forward_bundle returned a state %r outside the frozen "
                "bundle-state vocabulary" % value.state
            )
        return value

    def _validate_expiration(self, value: Any) -> Any:
        if not isinstance(value, tuple):
            return _ContractViolation(
                "expire_bundles must return a tuple of bundle refs"
            )
        for ref in value:
            ...
            try:
                validate_opaque_ref(ref, "bundle")
            except MeshError:
                return _ContractViolation(
                    "expire_bundles returned a malformed bundle ref; "
                    "value discarded"
                )
        return value

    def _validate_observation(self, value: Any) -> Any:
        if not isinstance(value, MeshObservation):
            return _ContractViolation(
                "observe_queue must return a MeshObservation"
            )
        # Generic metric vocabulary re-assert (mirrors the WORK-022
        # discipline): every sample metric must be the generic
        # WORK-016 link-metric vocabulary (the model enforces at
        # construction; the seam re-checks structurally).
        ...
        for sample in value.samples:
            ...
            if name not in valid_metrics:
                return _ContractViolation(
                    "observe_queue returned a sample metric %r outside "
    ...
    def _validate_app_session(self, value: Any) -> Any:
        if not isinstance(value, MeshAppSession):
            return _ContractViolation(
                "app_session must return a MeshAppSession instance "
        ...
        # standard session semantics; no ADCOS/mesh API may appear
        ...
        for method in _APPSESSION_METHODS:
            if not callable(getattr(value, method, None)):
                return _ContractViolation(
                    ...
                    "(connect/send/recv/close); NO ADCOS/mesh API in "
                    "the app path"
                )
        # Reject a leaky facade that exposes ADCOS/mesh tokens as
        # public attributes.  The facade's routing metadata is
        # underscore-prefixed; a leaky session exposing
        # session_id/bearer_ref/link_ref/binding_id/route_ref/... is
        # rejected at the seam (structurally enforced).
        try:
        ...
        if leaked:
            return _ContractViolation(
                ...
                "-- ADCOS/mesh tokens must not appear on the "
    ...
    # ------------------------------------------------------------------
    # Public mediated operations (the 16 contract operations)
    # ------------------------------------------------------------------

    def open(self, now: str) -> MeshOpResult:
    ...
    def close_link(self, now: str, *, link_ref: str) -> MeshOpResult:
        return self._mediate(
            now, "close_link",
            lambda ctx: self._implementation.close_link(
    ...
    def register_route(
        self, now: str, *, path: Any,
    ) -> MeshOpResult:
        return self._mediate(
            now, "register_route",
            lambda ctx: self._implementation.register_route(ctx, path=path),
            validate=self._validate_route_view,
        )

    def close_route(self, now: str, *, route_ref: str) -> MeshOpResult:
        return self._mediate(
            now, "close_route",
            lambda ctx: self._implementation.close_route(
                ctx, route_ref=route_ref,
            ),
            validate=self._validate_nothing,
        )
    ...
    def bind_session(
        self, now: str, *, session_id: str, route_ref: str,
        requirements: Optional[Mapping[str, Any]] = None,
    ) -> MeshOpResult:
        return self._mediate(
            now, "bind_session",
            lambda ctx: self._implementation.bind_session(
                ctx, session_id=session_id, route_ref=route_ref,
    ...
    def enqueue_bundle(
        ...
        prior_evidence: Any = (), hop_budget: int = 0,
    ) -> MeshOpResult:
        return self._mediate(
            now, "enqueue_bundle",
            lambda ctx: self._implementation.enqueue_bundle(
                ctx, bearer_ref=bearer_ref, payload=payload,
                prior_evidence=prior_evidence, hop_budget=hop_budget,
            ),
            validate=self._validate_bundle_view,
        )

    def forward_bundle(
        self, now: str, *, bundle_ref: str,
    ) -> MeshOpResult:
        return self._mediate(
            now, "forward_bundle",
            lambda ctx: self._implementation.forward_bundle(
                ctx, bundle_ref=bundle_ref,
            ),
            validate=self._validate_outcome,
        )

    def expire_bundles(self, now: str) -> MeshOpResult:
        return self._mediate(
            now, "expire_bundles",
            lambda ctx: self._implementation.expire_bundles(ctx),
            validate=self._validate_expiration,
        )
    ...
    def inspect_bundle(
        ...
        return self._mediate(
            now, "inspect_bundle",
            lambda ctx: self._implementation.inspect_bundle(
                ctx, bundle_ref=bundle_ref,
            ),
            validate=self._validate_bundle_view,
        )
    ...
    def observe_queue(self, now: str) -> MeshOpResult:
        return self._mediate(
            now, "observe_queue",
            lambda ctx: self._implementation.observe_queue(ctx),
