"""WORK-045 eligibility-family deep-immutability helpers.

Private family module (NOT part of the frozen public surface):
the structural enforcement behind the W053/W044 review-cycle
discipline, applied from day one in the eligibility family --
the public projections (trust records, capability declarations,
offer records, device signals, policy records, decision
records), the journaled command/event payloads, and the
idempotency-ledger entries are DEEPLY frozen, so a state
change without a journal append is structurally impossible,
not caller-honored.
...
- :func:`deep_materialize` converts a frozen (or plain)
  ...
  materialization is DIGEST-NEUTRAL: the canonical bytes of
  the frozen and the plain form of the same logical value are
...
def deep_freeze(value: Any) -> Any:
    ...
    -> tuples).  The result admits NO in-place mutation
