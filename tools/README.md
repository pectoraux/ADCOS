...
| `41-commit-token-required` | REGRESSION (PR #13 corrections 4-5): with a LEGITIMATE authority constructed, direct module-private commit invocations are rejected for every non-authority caller (None / random object / string / the session store itself / a dict / a foreign authority / the foreign store) with no mutation; ONLY the genuine authority instance commits; legitimate operations continue |
| `42-token-acquisition-surfaces` | REGRESSION (PR #13 correction 5): the accessor is removed from the module namespace; every module-level callable probed with the target store, every instance attribute, every session-store attribute, and every operation-result object is NON-committable; the store stays byte-identical; only the genuine authority instance commits |
