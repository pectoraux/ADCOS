...
| `41-commit-token-required` | REGRESSION (PR #13 correction 4): with a LEGITIMATE authority constructed, direct module-private commit invocations are rejected for every wrong-token shape (None / random object / string / fresh same-class token / another store's genuine token) with no mutation; ONLY the genuine token commits; legitimate MultipathStore operations continue afterward; the token is never an instance attribute |
